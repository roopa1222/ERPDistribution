...
import {
  createDsrInvoiceSchema,
  getDashBoardCountSchema,
  getDsrInvoiceSchema,
  updateDsrInvoiceSchema,
} from "../validator/dsrInvoice";
import {
  createDsrInvoice,
  getAccessoriesCount,
  getAllDsrInvoice,
...
export default class DsrInvoiceController {
    ...
    try {
      ...
      if (user?.role === IRoles.SALESMAN) {
        ...
        if (!req.body.branchId)
          return next(ApiError.customError(422, "branch is required"));
      ...
      if (!addDsrInvoice)
        return next(ApiError.customError(422, "DSR Invoice Not Created"));

      return res
        .status(200)
        .json({
          status: 200,
          data: { message: "DSR Invoice Created Successfully." },
          error: null,
        });
    } catch (e) {
      next(e);
  ...
  static updateDsrInvoice = async (
    ...
    try {
      ...
      if (!dsrInvoice)
        return next(ApiError.customError(422, "DSR Invoice Not Updated"));

      return res
        .status(200)
        .json({
          status: 200,
          data: { message: "DSR Invoice Updated Successfully." },
          error: null,
        });
    } catch (e) {
      return next(e);
  ...
  static getDsrInvoiceDetails = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
    try {
      ...
      const dsrData = await getAllDsrInvoice(
        result.searchValue,
        result.branchId,
        result.startDate,
        result.endDate,
        parseInt(result.limit, 10),
        parseInt(result.offset, 10)
      );
  ...
  static getDSRInvoiceExcelData = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
      ...
      const formattedData = dsrData?.dsrData?.flatMap((item) => {
        ...
        if (Array.isArray(item.paymentDetails)) {
          item.paymentDetails.forEach((payment: any, index: any) => {
            ...
            paymentColumns[`paymentAmount_${index + 1}`] =
              payment.amount || null;
          });
        }
        ...
        if (Array.isArray(item.financeDetails)) {
          item.financeDetails.forEach((finance: any, index: any) => {
            financeColumns[`financeType_${index + 1}`] =
              finance.financeName || null;
            financeColumns[`financeAmount_${index + 1}`] =
              finance.amount || null;
          });
        }
  ...
  static getDashBoardCountData = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
    try {
      ...
      const mobileCount = await getMobileCount(
        result.branchId,
        result.from,
        result.to
      );
      const accessoriesCount = await getAccessoriesCount(
        result.branchId,
        result.startDate,
        result.endDate
      );
      const electronicCount = await getElectronicCount(
        result.branchId,
        result.from,
        result.to
      );
