...
export default class DailyexpenseController {
  ...
  static getDailyOpeningAndClosingBalanceDetails = async (req: Request, res: Response, next: NextFunction) => {
    try {
       ...
       console.log('step01')
       const getDailyExpense = await getDailyBalanceDetails(result.branchId, result.from, result.to);
       if (!getDailyExpense) return next(ApiError.customError(404, 'Daily Opening/Closing Balance Not Found'));
